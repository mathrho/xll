...
if strcmp(type, 'onevsall')
    ...
    parfor ci = 1 : svmmodel.nr_class
	    ...
	    dataset = vl_maketrainingset(data, int8(lab)) ;
            w(:, ci) = vl_svmpegasos(dataset, lambda, ... 
                                      'MaxIterations', length(lab) * 100, 'BiasMultiplier', biasMultiplier) ;
	else
            label = labels(:, ci) ;
            lab = label(label ~= 0) ;
            dat = data(:, label ~= 0) ;
            ...
            %                      'NumIterations', length(lab) * 100, 'BiasMultiplier', biasMultiplier) ;
	    dataset = vl_maketrainingset(dat, int8(lab)) ;
	    w(:, ci) = vl_svmpegasos(dataset, lambd, ... 
                                      'MaxIterations', length(lab) * 100, 'BiasMultiplier', biasMultiplier) ;
        end
    end
